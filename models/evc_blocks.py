...
class EVCBlock(nn.Module):
    def __init__(self, in_channels, out_channels, channel_ratio=4, base_channel=16):
        ...
        # self.linear1 = nn.Linear(in_channels=1, out_channels=256)
        # self.linear2 = nn.Linear(in_channels=256, out_channels=1)

    def forward(self, x):
        # x = x.unsqueeze(1).repeat(1,768,768,1)
        x = x.repeat(1,256,1)
        B, HW, C = x.shape
        x1 = x.permute(0, 2, 1).reshape(B,C,int(math.sqrt(HW)),int(math.sqrt(HW)))
        ...
        x = (x_lvc + x_lmlp) / 2
        x =  x.reshape(B,C,HW).permute(0,2,1).mean(dim=1, keepdim=True)
        # x = self.cnv1(x)
        return x