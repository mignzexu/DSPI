...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, patch_embedding, img_embedding, gt_text_embedding_map, noise_text_embeddings, gt_density):
        ...
        """
        gt_density = F.interpolate(gt_density.unsqueeze_(1), size=(224, 224), mode='nearest') #([32, 1, 224, 224])
        density_mask = F.max_pool2d(gt_density, kernel_size=16, stride=16, padding=0) #same as ViT conv1  ([32, 1, 14, 14])
        density_mask = density_mask > 0.
        density_mask = density_mask.permute(0, 2, 3 ,1) # (B, 14, 14, 1) ([32, 14, 14, 1])
        ...
        fused_text_embedding_map =  gt_text_embedding_map #([32, 14, 14, 512])
        pos_mask = density_mask.squeeze_(-1) # (B, 14, 14, 1) ([32, 14, 14])
        
        patch_embeddings = patch_embedding.reshape(-1, 14, 14, 512)  #([32, 14, 14, 512])
        #batch cosine similarity, this function automatically normalizes the vectors
        sim_map = F.cosine_similarity(patch_embeddings, fused_text_embedding_map , dim=-1) # (B, 14, 14) ([32, 14, 14])
        # sim_global = F.cosine_similarity(img_embedding, fused_text_embedding_map , dim=-1) # (B, 1)
        n_pos = torch.sum(pos_mask, dim=(1, 2)) # (B) how many positive samples in each batch #([32])
        # if n_pos == 0, set to 1 to avoid nan
        n_pos = torch.where(n_pos == 0, torch.ones_like(n_pos), n_pos)
        #infoNCE 

        sim_map = torch.exp(sim_map / self.temperature)  #([32, 14, 14])
