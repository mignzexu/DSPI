...
class FSC147(Dataset):
    def __init__(self, 
        ...
        #!HARDCODED Dec 25: 
        self.args = args
        self.data_dir = f"{self.args.data_path}/FSC/"
...
class FSC147_gdino(FSC147):
    ...
    def __getitem__(self, idx):
        ...
        if "test" in self.split or (self.split == "val" and not self.resize_val):
            if not self.args.online:
            ...
            vars_to_return.extend([gdino_logits, img_name])
...
if __name__ == "__main__":
    fsc_dataset_train = FSC147(split = "train")
    fsc_gdino_dataset_train = FSC147_gdino(split = "train")
    idx = 0 
    sample = fsc_gdino_dataset_train[idx]

    image = sample[0]  
    gdino_box = sample[-2]
    gdino_logit = sample[-1]

    """
    Image shape: torch.Size([3, 384, 384])
    Gdino Box: torch.Size([16, 4])
    Gdino Logit: torch.Size([16])
    """
    print("Image shape:", image.shape)
    print("Gdino Box:", gdino_box.shape)
    print("Gdino Logit:", gdino_logit.shape)
