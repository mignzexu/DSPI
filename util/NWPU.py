...
class NWPU(Dataset):
    def __init__(self, split, part=None, args=None):
        ...
        self.split = split
        self.args = args
        self.data_dir = f"{self.args.data_path}/NWPU/{self.split}"

        self.img_paths = glob.glob(os.path.join(self.data_dir, "imgs/*.jpg"))
        ...
        if self.split != "test":
            for img_path in self.img_paths:
                ...
                gt_path = img_path.replace("imgs", "mats").replace(".jpg", ".mat")

                with open(gt_path, "rb") as f:
                    mat = loadmat(f)
                    # the number of count is lenth of the points
                    self.gt_cnt[img_name] = len(mat["annPoints"])
